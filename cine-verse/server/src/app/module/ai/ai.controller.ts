...
const chatWithBot = async (req: Request, res: Response) => {
  try {
    ...
    const moviesContext = allMovies.map(
      (m) =>
        `${m.title} (${m.releaseYear || "N/A"}) - ${m.genre.slice(0, 2).join(", ")}`,
    );
...
const getRecommendations = async (req: Request, res: Response) => {
  try {
    ...
    try {
      const recommendedTitles = await aiService.generateMovieRecommendation(
        userInterests,
        movieTitles,
        prompt,
      );
...
const generateSynopsis = async (req: Request, res: Response) => {
  try {
    ...
    const synopsis = await aiService.generateSynopsis(
      title,
      director,
      type || "movie",
      genre || "general",
