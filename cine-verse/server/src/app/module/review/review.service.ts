...
const createReview = async (userId: string, userRole: string, payload: any) => {
  ...
  // Auto-approve all reviews - admin will handle spoiler warnings
  let status: ReviewStatus = ReviewStatus.APPROVED;

  // Only use AI sentiment analysis for extremely inappropriate content
  try {
    ...
    // Keep APPROVED status if AI fails
  }
  ...
  return {
    ...review,
    message: (status as ReviewStatus) === ReviewStatus.PENDING 
