...
export default function HomePage() {
  ...
  const loadData = async () => {
    try {
      ...
      setNewReleases(Array.isArray(nRes) ? nRes : (nRes?.data || []));
      ...
      setTimeout(async () => {
        try {
          const [cRes, eRes, rRes, aRes] = await Promise.all([
          ...
          setComingSoon(Array.isArray(cRes) ? cRes : (cRes?.data || []));
          setEditorsPicks(Array.isArray(eRes) ? eRes : (eRes?.data || []));
          setRecentReviews(Array.isArray(rRes) ? rRes : (rRes?.data || []));
