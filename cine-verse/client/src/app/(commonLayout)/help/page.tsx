...
import { HelpCircle, Search, Play, CreditCard, User, Shield, ArrowRight, MessageCircle } from "lucide-react";
...
export default function HelpPage() {
  ...
  const categories = [
    { title: "Getting Started", icon: Play, items: ["Account setup", "Device compatibility", "Streaming quality", "Supported browsers"] },
    { title: "Billing & Plans", icon: CreditCard, items: ["Managing subscription", "Invoices and receipts", "Payment methods", "Refund policy"] },
    { title: "Account & Profile", icon: User, items: ["Changing password", "Linking social accounts", "Profile customization", "Parental controls"] },
    { title: "Privacy & Safety", icon: Shield, items: ["Data protection", "Reporting content", "Two-factor auth", "Cookie settings"] },
  ];

  const faqs = [
    { q: "How many devices can I stream on simultaneously?", a: "Premium members can stream on up to 4 devices at once. Standard members are limited to 2 devices." },
    { q: "Can I download movies for offline viewing?", a: "Yes! Use the CineVerse mobile app to download titles and watch them anywhere without an internet connection." },
    { q: "How does the AI recommendation work?", a: "CineBuddy analyzes your watch history, ratings, and even the time of day you watch to suggest the perfect movie for your mood." },
    { q: "What should I do if the video keeps buffering?", a: "Ensure you have a stable connection of at least 5Mbps for HD or 25Mbps for 4K. Try clearing your browser cache or restarting the app." },
  ];
    ...
    <div className="min-h-screen bg-black text-white pt-32 pb-20 selection:bg-purple-500/30">
      <div className="container mx-auto px-6">
          ...
          {categories.map((cat, i) => (
            ...
            >
              ...
              <ul className="space-y-3">
                {cat.items.map(item => (
                  <li key={item} className="text-xs text-gray-500 hover:text-purple-400 transition-colors flex items-center gap-2">
            ...
            {faqs.map((faq, i) => (
              ...
              >
                <h4 className="font-bold text-lg mb-4 flex justify-between items-center group-hover:text-purple-400">
                  {faq.q}
                  <span className="text-gray-700 font-normal group-hover:text-purple-500 text-2xl leading-none">+</span>
                </h4>
        ...
        <section className="text-center p-12 md:p-20 rounded-[40px] bg-gradient-to-tr from-zinc-900 to-black border border-white/5 relative overflow-hidden">
          ...
          <h2 className="text-3xl font-black italic uppercase mb-4 relative z-10">Still need help?</h2>
          <p className="text-gray-500 max-w-xl mx-auto mb-10 relative z-10">
            Our support crew is always on standby. Whether you need technical help or just a movie recommendation, we've got you covered.
          </p>
