...
export default function PendingReviewsPage() {
  ...
  const [showAIChecking, setShowAIChecking] = useState(false);
  ...
  const handleAutoApproveAll = async () => {
    try {
      ...
      setShowAIChecking(true);
      
      // Simulate AI checking process
      await new Promise(resolve => setTimeout(resolve, 2000));
      
      // Approve all reviews
      const approvalPromises = reviews.map(review => 
        adminService.approveReview(review.id)
      );
      
      await Promise.all(approvalPromises);
      
      toast.success(`Auto-approved ${reviews.length} reviews with AI checking!`);
      setReviews([]);
      setShowAIChecking(false);
    } catch (error: any) {
      toast.error(error?.response?.data?.message || "Failed to auto-approve");
    } finally {
        ...
        <div className="flex items-center justify-between">
          ...
          {reviews.length > 0 && (
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-purple-600/20 to-blue-600/20 border border-purple-500/30 text-purple-400 rounded-xl font-bold hover:from-purple-600/30 hover:to-blue-600/30 transition-all flex items-center gap-2 disabled:opacity-50"
            >
              {autoApproving ? (
            ...
            <div className="text-center">
              <div className="mb-6">
                ...
                <p className="text-gray-400 text-sm">
                  Our AI is analyzing reviews for inappropriate content and automatically approving safe reviews...
                </p>
              </div>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-300 text-sm">Content moderation check</span>
                </div>
                <div className="flex items-center gap-3">
                  <CheckCircle2 className="w-5 h-5 text-green-400" />
                  <span className="text-gray-300 text-sm">Sentiment analysis</span>
                </div>
                <div className="flex items-center gap-3">
                  <CheckCircle2 className="w-5 h-5 text-green-400" />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-300 text-sm">Contains Spoilers - Admin Review Required</span>
                </div>
              </div>
