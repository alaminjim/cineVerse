                  ...
                  {(field) => (
                    <div className="sm:col-span-2">
                      <div className="flex items-center justify-between mb-2">
                        <label className="text-xs font-bold uppercase tracking-widest text-gray-500 block">Synopsis *</label>
                        <button
                          type="button"
                          onClick={async () => {
                            const title = form.getFieldValue("title");
                            const director = form.getFieldValue("director");
                            const type = form.getFieldValue("type");
                            const genre = form.getFieldValue("genre");
                            
                            if (!title || !director) {
                              toast.error("Please enter Title and Director first!");
                              return;
                            }
                            
                            try {
                              setIsGenerating(true);
                              const res = await aiService.generateSynopsis(title, director, type, genre);
                              if (res.success) {
                                field.handleChange(res.data);
                                toast.success("Synopsis generated by AI!");
                              }
                            } catch (error) {
                              toast.error("AI failed to generate synopsis.");
                            } finally {
                              setIsGenerating(false);
                            }
                          }}
                          disabled={isGenerating}
                          className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-purple-400 hover:text-purple-300 transition-colors disabled:opacity-50"
                        >
                          {isGenerating ? (
                      ...
                      <textarea
                        ...
                        placeholder="Once you enter title and director, click 'AI Generate' for a quick start!"
                        required
                      />
                    </div>
                  )}
